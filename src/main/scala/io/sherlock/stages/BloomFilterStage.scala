 ...
 */
class BloomFilterStage(val numHashes: Int = 8, val width: Int = 1024)
    ...
    new GraphStageLogic(shape) {
      val requestHandler: HttpRequest ⇒ HttpResponse = {
        case HttpRequest(
              ...
              Uri.Path("/put"),
              headers,
              Strict(ContentTypes.`text/plain(UTF-8)`, data),
        ...
        case HttpRequest(HttpMethods.GET, uri, headers, entity, HttpProtocols.`HTTP/1.1`)
            if uri.path.startsWith(pathPref) ⇒
